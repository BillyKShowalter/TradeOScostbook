...
def complete_task(task_id):
    ...
    completed_list = load_json(COMPLETED_PATH, [])
    ...
    # Remove from queue, add to completed list
    queue.pop(target_index)
    completed_list.append(target_task)
    
    save_json(QUEUE_PATH, queue)
    save_json(COMPLETED_PATH, completed_list)
    ...
    print(f"[{datetime.now().isoformat()}] Task {task_id} successfully marked COMPLETED and archived.")

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 scripts/complete-task.py <task-id>")
        sys.exit(1)
