...
- **(Undocumented until item 41 below)** `web/`'s auth model has since migrated fully to **Supabase Auth** (`@supabase/ssr`), replacing the httpOnly-JWT-cookie scheme described above; a `project-intake` backend module (AI-assisted site-visit classification/confidence scoring) and matching `web/` UI page also shipped without a log entry
- A legacy Costbook/Swift/Knowledge-Engine asset migration landed at `packages/knowledge-engine/` (~76MB of data/pipelines/prompts/review workflow) and `packages/legacy-tradeos-reference/` (transfer-planning docs) — see `docs/PROJECT_STATUS.md` and `docs/KNOWLEDGE_ENGINE_STATUS.md` for current, audited status; it is not yet wired into `app/` or `web/`
- Rolling notes exist in:
  - `docs/rolling-todo.md`
  - `docs/end-of-session-note.md`
  - `docs/PROJECT_STATUS.md` and `docs/EXECUTIVE_REPOSITORY_AUDIT.md` (current, audited state — prefer these over the bullets above if they conflict)
...
41. **Gap note (written retroactively, during the repository-audit session below):** between item 40 and this entry, the repo gained several commits that were never narrated here: the `project-intake` module (deterministic AI site-visit classifier + confidence scoring, persistence, and a `POST /api/v1/project-intake/classify` endpoint, across three commits), `docs/TradeOS-Engineering-Playbook.md`, `docs/AI_ESTIMATING_ARCHITECTURE.md` (the Photos→Voice→...→Closeout pipeline design), and — most significantly — a full migration of `web/`'s auth model from the hand-rolled JWT-cookie scheme described in items 37–38 to **Supabase Auth** (`@supabase/ssr`); `web/` also gained a `/projects/[id]/intake` UI page for the new module. None of this is detailed here because it happened in session(s) that didn't update this file — a process gap, flagged in the audit below. Read the actual code/tests as ground truth for this period, not this log.

42. **Full repository audit** following the migration of legacy Costbook/Swift/Knowledge-Engine assets into `packages/knowledge-engine/` (data/pipelines/prompts/review workflow/scripts/docs/agent definitions, ~76MB) and `packages/legacy-tradeos-reference/` (5 transfer-planning docs). Produced nine new docs under `docs/`: `PROJECT_STATUS.md`, `REPOSITORY_STRUCTURE.md`, `SYSTEM_ARCHITECTURE.md`, `KNOWLEDGE_ENGINE_STATUS.md`, `LEGACY_ASSET_MIGRATION_STATUS.md`, `NEXT_STEPS.md`, `TECHNICAL_DEBT.md`, `MIGRATION_AUDIT.md`, and `EXECUTIVE_REPOSITORY_AUDIT.md` (start there for the summary), plus `packages/knowledge-engine/README.md` (that package had none at its root). Fixed stale `api/`→`backend/` path references and undercounted module/migration lists in `app/README.md` and the `run-tradeos-costbook-api` skill; corrected `web/README.md`'s auth-model description to match the real Supabase-based implementation (including flagging now-dead `signup()`/`login()` helpers in `src/lib/api.ts`) and its broken `.env.example` setup instruction; clarified `packages/legacy-tradeos-reference/README.md`'s self-referential naming now that the payload and the docs live in separate directories; added `packages/` to the root `README.md`'s structure diagram. Verified no duplicate knowledge-engine payload exists in the repo (a near-duplicate copy was caught via byte-diff before being committed in an earlier session). Flagged but did not fix (needs a maintainer/domain-expert decision, not a mechanical edit): `docs/frontend-platform-completion-plan.md` still says "no login UI yet" (false), and `docs/AI_ESTIMATING_ARCHITECTURE.md` was written without apparent awareness of the knowledge-engine corpus that landed the same day — see `docs/NEXT_STEPS.md` item 1. Flagged for cleanup, not executed (audit scope was docs, not repo surgery; nothing was deleted): a git-tracked npm cache (`packages/knowledge-engine/legacy-archive/scratch/.npm_cache/`) and a ~66M generic third-party AI-skills marketplace mixed into `packages/knowledge-engine/agent-skills/skills/` that has no relation to the construction/costbook domain — full detail in `docs/TECHNICAL_DEBT.md`.

## Next Suggested Slice
See `docs/NEXT_STEPS.md` for the current, audited list — it supersedes the bullets that used to live here (several were already resolved: the `api/`→`backend/` rename is committed, `vercel.json` is intentional, Phase 1 of the front-end plan shipped). Highlights carried forward:
- Reconcile `docs/AI_ESTIMATING_ARCHITECTURE.md` with the now-present `packages/knowledge-engine/` corpus before building any retrieval/reasoning layer from scratch.
...
- Resolve why required-reviewer protection rules won't enable on the `production` GitHub Environment despite GitHub Pro.
- Add a real `SupplierFeedFetcher` implementation once an actual supplier feed exists to integrate with.
