...
Parsed into searchable assemblies/cost items:
...
Recursively inventoried (file paths/counts only, for `stats.sourceFileCount`) but **not parsed into search or matching**:
...
The per-trade assembly/cost-item files carry richer content (proposal language, risk factors, permit/safety notes) than `costbook.json`. See `docs/KNOWLEDGE_ENGINE_RUNTIME_INTEGRATION.md` for why this gap matters and what parsing them would take.

## Runtime behavior

- File-based loading only
- In-memory cached snapshot
- Deterministic keyword and metadata scoring (a small stopword filter excludes function words like "and"/"the" from keyword matching)
- Human-review-first match output

## Future work

- Parse the per-trade `knowledge/assemblies/<trade>/*.json` and `knowledge/cost-items/<trade>/*.json` files into search/matching instead of only counting them (see Data sources above)
